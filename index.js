/**
 * Context Panel Extension for SillyTavern
 *
...
async function loadMemories(container, amCodes) {
    ...
    if (!api) {
        console.warn('[ContextPanel] AutoCardUpdaterAPI not available');
        if (listEl) {
    ...
    try {
        if (api.refreshDataAndWorldbook) {
            try {
                ...
                console.warn('[ContextPanel] Data refresh failed:', e);
            }
        }
        ...
        console.log('[ContextPanel] Tables:', Object.values(tables).map(t => t?.name).filter(Boolean));
        ...
        console.error('[ContextPanel] Memory loading failed:', err);
        if (listEl) {
            listEl.innerHTML = '<div class="error-message">'
...
export function init() {
    eventSource.on(event_types.USER_MESSAGE_RENDERED, processMessage);
    eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, (messageId) => processMessage(messageId));
    eventSource.on(event_types.MESSAGE_UPDATED, processMessage);

    // Process existing messages when more history is loaded
    ...
    console.log('[ContextPanel] Extension loaded');
}
